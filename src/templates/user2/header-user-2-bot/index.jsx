...
function HeaderUser2Bot() {
	...
	const setActive = () => {
		...
		const allRootMenu = document.querySelectorAll(
			'[data-header-parent="parent-header"]',
		);
		...
		allRootMenuArray.forEach((item) => {
			item?.querySelector(
				'[data-header-parent-title="title"]',
			)?.classList?.remove(blueClass);
		});

		// set active base current url
		const currentHeader = allHeadersArray?.find(
			(item) => item.dataset.header === location.pathname,
		);
		currentHeader?.classList?.add(blueClass);
		const currentHeaderParent = currentHeader
			?.closest('[data-header-parent="parent-header"]')
			?.querySelector('[data-header-parent-title="title"]');
		currentHeaderParent?.classList?.add(blueClass);
	};
	const closeMenu = (subItem) => {
		const currentContainer = subItem
			?.closest("[data-header-parent='parent-header']")
			?.querySelector("[data-menu='menuContainer']");
	...
	return (
		<div className={css.headerUser2Bot}>
			<div className={css.container}>
				...
				<ul
					className={`${css.headerUser2Bot__menu} ${renderShowMenu()}`}
				>
					...
					<li
						onClick={menuCLickHandle.bind(
							null,
							menus.current.blockChain,
						)}
					>
						<DropdownHeader3
					...
					<li
						onClick={menuCLickHandle.bind(null, menus.current.nFTs)}
					>
						<DropdownHeader3
					...
					<li
						onClick={menuCLickHandle.bind(null, menus.current.more)}
					>
						<DropdownHeader3
							...
							menuWidthFull={true}
							content={
								<MoreContent redirectPage={redirectPage} />
							}
						/>
					</li>
				</ul>
				...
				<div className={css.headerUser2Bot__bar}>
					<Button2
						classname={css.headerUser2Bot__bar__custom}
