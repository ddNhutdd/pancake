...
function CopyButton(props) {
	...
	const onCopyCLickHandle = () => {
		...
		!copyElement.current.classList.contains('d-0') &&
			copyElement.current.classList.add('d-0');
		tickElement.current.classList.remove('d-0');

		idTimeout.current = setTimeout(() => {
			copyElement.current.classList.remove('d-0');
			!tickElement.current.classList.contains('d-0') &&
				tickElement.current.classList.add('d-0');
