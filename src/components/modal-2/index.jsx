...
function Modal2(props) {
	...
	const closeModal = () => {
		...
		!modalElement.current.classList.contains(css.modalOut) &&
			modalElement.current.classList.add(css.modalOut);

		!overlayElement.current.classList.contains(css.fadeOut) &&
			overlayElement.current.classList.add(css.fadeOut);
			...
			>
				...
				>
					<Card>
						...
						<div
							className={`${css.modal__content} ${classNameContent}`}
						>
							{content}
						</div>
						<div
							className={`${css.modal__footer} ${renderClassShowFooter()} ${classNameFooter}`}
						>
							{footer}
						</div>
					</Card>
