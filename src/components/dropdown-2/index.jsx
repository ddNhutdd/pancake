...
export const Dropdown2 = (props) => {
	...
	const renderLine = (lineBot) =>
		lineBot === true ? (
			<div className={css.dropdown2__item__line}></div>
		) : (
			<></>
		);
	const renderClassAllowItemHover = (values) =>
		values === false ? css.itemNotAllowHover : '';
	...
	return (
		...
		>
			<div
				ref={headerElement}
				className={css.dropdown2__header}
			>
				{header}
			</div>
			<div
			...
			>
				<div className={css.dropdown2__menu}>{renderListItem()}</div>
			</div>
		</div>
	);
};
