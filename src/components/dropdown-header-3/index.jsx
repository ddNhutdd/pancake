...
export const DropdownHeader3 = (props) => {
	...
	const renderMenuItem = () => {
		...
		return list.map((item) => (
			...
			>
				<div
					data-header={item.url}
					className={css.dropdownHeader3__item}
				>
					{item.content}
				</div>
	...
	return (
		<div
			...
			className={`${css.dropdownHeader3} ${renderClassMenuWidthFullHeader()}`}
		>
			<div
				data-header-parent-title={`title`}
				onClick={headerClickHandle}
				className={`${css.dropdownHeader3__header} ${cssHeader}`}
			>
				{header}
				<FaAngleDown />
			</div>
			<div
				data-menu={`menuContainer`}
				className={`${css.dropdownHeader3__menuContainer} ${renderNotShowMenu()} ${renderClassMenuWithFullContainer()} ${renderClassShowMenu()}`}
			>
