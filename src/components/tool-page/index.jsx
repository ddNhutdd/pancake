...
function ToolPage(props) {
	...
	const renderClassMenuActive = (url) =>
		menuActive === url ? css.active : '';
	const redirectPage = (url) => navigate(url);
	const toggleMenu = () => setIsShowMenuToolInMoble((state) => !state);
	const renderClassShowMenuInMoble = () =>
		isShowMenuToolInMoble ? css.toolPage__menuMoble : '';
			...
			<div className={css.container}>
				...
				<div className='pt-3 row'>
					<div className='col-2 border-r-1 border-r-sm-0  col-lg-4 col-sm-12'>
						...
						<ul
							className={`${css.toolPage__menu} ${renderClassShowMenuInMoble()}`}
						>
							{renderListMenu()}
						</ul>
					</div>
					<div className='col-10 col-lg-8 col-sm-12'>{children}</div>
				</div>
			</div>
		</div>
	);
