...
function Card(props) {
    const { img, title, smallTitle, number, info, action, outline_button } = props;
    ...
    const renderStyle = () => {
        return outline_button ? buttonClassesType.outline : buttonClassesType.primary;
    }
    ...
    return (
        <div className={`${css.card} ${renderDarkTheme()}`}>
            ...
            <ul className={css.card__info}>
                {renderInfoRecord(info)}
            </ul>
            <div className={`${css.card__action} ${renderUnderline()}`}>
                <Button isDark={renderButtonDark()} type={renderStyle()} className={`w-100 `}>{action}</Button>
            </div>
        </div >
    )
}
