...
const Dropdown = function () {
			...
			left: (
				<div>
					<div className={css.dropdown__itemLeft}>
						<img
							src='https://bscscan.com/token/images/ethereum_32.png'
							alt='ethereum'
						/>
						Binance-Peg ... (ETH)
					</div>
					<div className={`--text-gray`}>0.00404291 ETH</div>
				</div>
			),
			right: (
				<div>
					<div>$13.44</div>
					<div className={`--text-gray`}>@3,323.6243</div>
				</div>
			),
		},
		{
	...
	//#region function
	const renderListItemDropdown = () =>
		listDropdownItem.map((item) => (
			<DropdownItem
				key={item.id}
				content={item}
			/>
