...
const Block = function () {
	...
	const cardCollapseElement = useRef(null);

	const renderClassShowMore = () => showMore ? '' : 'd-0';
	const renderClassShowLess = () => showMore ? 'd-0' : ''; 
	const toggleInfoClickHandle = () => setShowMore(s => !s);
	const renderClassWhenShowMore = () => !showMore ? css['--mt0'] : '';
	...
	const renderContent = () => {
		...
		<Card className={css.block__card}>
			<div className={css.block__cardContainer}>
		...
		<Card className={css.block__card}>
			<div 
				ref={cardCollapseElement} 
				className={`${css.block__cardContainer}`}
			>
				<div className={css.block__cardRecord}>
					<div className={css.block__left}>
						<Popover
				...
				<div className={css.block__line}></div>
				<div className={css.block__last}></div>
			</div>
			<div className={`${css.block__lastRow} ${renderClassWhenShowMore()}`}>
				<div className={css.block__left}>
					More Details:
				</div>
				<div onClick={toggleInfoClickHandle} className={css.block__right}>	
					<div className={renderClassShowLess()}>
						<FaPlus />
					</div>
					<div className={renderClassShowMore()}>
						<FiMinus />
					</div>
					<div className={renderClassShowLess()}>
						Click to show more
	...
	useEffect(()=>{
		if (!showMore) {
			if (!cardCollapseElement || !cardCollapseElement.current) return;
			cardCollapseElement.current.classList.add(css.close);
			cardCollapseElement.current.classList.remove(css.show);
			const idTimeout = setTimeout(()=>{
				cardCollapseElement.current.style.display = 'none'
				cardCollapseElement.current.classList.remove(css.close);
				clearTimeout(idTimeout);
			}, 400);
		} else {
			cardCollapseElement.current.style.display = 'flex'
			cardCollapseElement.current.classList.add(css.show);
		}
	}, [showMore])
	useEffect(() => {
		fetchInfo();
